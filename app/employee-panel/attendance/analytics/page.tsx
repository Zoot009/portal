          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Your work patterns and productivity metrics</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-medium">vs Required</span>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="flex-1">
                      <div className="text-2xl font-bold text-blue-600">{formatHoursToTime(avgDailyHours)}</div>
                      <div className="text-xs text-muted-foreground">per day</div>
                    </div>
                    <div className="text-lg text-muted-foreground">vs</div>
                    <div className="flex-1">
                      <div className="text-2xl font-bold text-purple-600">{formatHoursToTime(minutesToHours(dailyGoalMinutes))}</div>
                      <div className="text-xs text-muted-foreground">required</div>
                    </div>
                  </div>
                  {avgDailyHours >= minutesToHours(dailyGoalMinutes) ? (
                    <div className="bg-green-50 border border-green-200 rounded-lg p-2 text-center">
                      <span className="text-xs font-medium text-green-700">
                        🎯 You're exceeding daily target by {formatHoursToTime(avgDailyHours - minutesToHours(dailyGoalMinutes))}!
                      </span>
                    </div>
                  ) : (
                    <div className="bg-orange-50 border border-orange-200 rounded-lg p-2 text-center">
                      <span className="text-xs font-medium text-orange-700">
                        ⚡ Work {formatHoursToTime(minutesToHours(dailyGoalMinutes) - avgDailyHours)} more per day to meet target
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-3 border-t">
                  <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg p-3 border border-green-200">
                    ...
                    <div className="text-xs text-green-600 mt-1">
                      {stats.present}/{stats.totalRecords} days
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-purple-50 to-violet-50 rounded-lg p-3 border border-purple-200">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs font-medium text-purple-700">Goal Days</span>
                    </div>
                    <div className="text-2xl font-bold text-purple-600">{stats.daysMetGoal}</div>
                    <div className="text-xs text-purple-600 mt-1">
                      of {stats.daysWithHours} days
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg p-3 border border-blue-200">
                    <div className="flex items-center gap-2 mb-1">
                      <TrendingUp className="h-4 w-4 text-blue-600" />
                      <span className="text-xs font-medium text-blue-700">Overtime</span>
                    </div>
                    <div className="text-2xl font-bold text-blue-600">{formatHoursToTime(overtimeHours)}</div>
                    <div className="text-xs text-blue-600 mt-1">
                      {stats.daysWithOvertime} days OT
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-lg p-3 border border-orange-200">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs font-medium text-orange-700">Issues</span>
                    </div>
                    <div className="text-2xl font-bold text-orange-600">{stats.absent + stats.late}</div>
                    <div className="text-xs text-orange-600 mt-1">
                      {stats.absent}A · {stats.late}L
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-3 border-t">
                  <div className="text-xs font-medium text-muted-foreground mb-2">💡 Quick Insights</div>
                  <div className="space-y-1.5">
                    {stats.daysWithHours > 0 && stats.daysMetGoal === stats.daysWithHours && (
                      <div className="flex items-start gap-2 text-xs text-green-700 bg-green-50 rounded p-2">
                        <span>✓</span>
                        <span>Perfect consistency! You're meeting daily goals every day.</span>
                      </div>
                    )}
                    {progressPercentage < 50 && workingDays - stats.daysWithHours > 0 && (
                      <div className="flex items-start gap-2 text-xs text-blue-700 bg-blue-50 rounded p-2">
                        <span>→</span>
                        <span>Focus on consistency. {workingDays - stats.daysWithHours} working days remaining.</span>
                      </div>
                    )}
                    {stats.absent > 0 && (
                      <div className="flex items-start gap-2 text-xs text-orange-700 bg-orange-50 rounded p-2">
                        <span>!</span>
                        <span>{stats.absent} absent day{stats.absent > 1 ? 's' : ''} - try to minimize absences.</span>
                      </div>
                    )}
                    {avgDailyHours > minutesToHours(dailyGoalMinutes) && progressPercentage < 50 && (
                      <div className="flex items-start gap-2 text-xs text-purple-700 bg-purple-50 rounded p-2">
                        <span>↑</span>
                        <span>Great pace! Keep it up to finish ahead of schedule.</span>
                      </div>
                    )}
                  </div>
                </div>
